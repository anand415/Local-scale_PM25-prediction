...
allerrstat=pickle.load(open("allerrstat_4c_10.pkl","rb"))
...
with open("comp4Cchk.txt",'w',encoding='utf-8') as file1:
    for ii in range(0,5):
          file1.write("{0} & {1} & {2:3.2f} &{3:3.2f} & {4:3.2f}& {5:3.2f}  \\\\".format(fstcol[ii],'Train',
                      stats_trnsrt[ii,0],stats_trnsrt[ii,1],stats_trnsrt[ii,2], stats_trnsrt[ii,3])) 
          file1.write('\n')
          file1.write(" & {0} & {1:3.2f} &{2:3.2f} & {3:3.2f} & {4:3.2f} &\\\\".format('Test',
                      stats_tstsrt[ii,0],stats_tstsrt[ii,1],stats_tstsrt[ii,2], stats_tstsrt[ii,3]))         
