...
pipe = Pipeline([
        ...
        ('reg', SVR(kernel='rbf', C=750, gamma=76,epsilon=0.17))])# svr = SVR(kernel='rbf', C=1e1, gamma=0.1)
...
[xgbp,tt]=pickle.load(open("xgb_300_B4C.pkl","rb"))
xgb= XGBRegressor(random_state=24,n_jobs=-1,n_estimators=300,**xgbp)

[lgbp,tt]=pickle.load(open("lgb_1000_B4C.pkl","rb"))
lgb = LGBMRegressor(random_state=24, n_jobs=-1,n_estimators=1000,**lgbp)

[cbp,tt]=pickle.load(open("cb_200_B4C.pkl","rb"))
...
data = {'scores4Bc' : scores}
m4p.savemat('scores4Bc.mat', data)   
...
pickle.dump(np.argsort(-fnl[:,2]),open("model_seq_4BC.pkl","wb")) 
...
# xgb,lgb,cbr,rf
with open("compB4C.txt",'w',encoding='utf-8') as file1:
    for ii in range(0,5):
          file1.write("{0} & {1} & {2:3.2f} &{3:3.2f} & {4:3.2f}& {5:3.2f} &{6:3.2f} \\\\".format(fstcol[ii],'Train',
                      stats_trn[ii,0],stats_trn[ii,1],stats_trn[ii,2], stats_trn[ii,3], stats_trn[ii,4])) 
          file1.write('\n')
          file1.write(" & {0} & {1:3.2f} &{2:3.2f} & {3:3.2f} & {4:3.2f} &{5:3.2f}\\\\".format('Test',
                      stats_tst[ii,0],stats_tst[ii,1],stats_tst[ii,2], stats_tst[ii,3], stats_tst[ii,4]))         
